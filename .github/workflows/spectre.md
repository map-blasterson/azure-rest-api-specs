...
2. **Hero scenarios** — detailed end-to-end scenarios that show a
   ...
   - **Persona**: who is performing this task and why
   - **API call sequence**: the ordered HTTP flow showing method, path,
     ...
     Azure-AsyncOperation: https://management.azure.com/...operationStatuses/{id}
     ```

     ```http
     ### Step 2 — Add a SQL Database target to monitor
     PUT .../watchers/{name}/targets/{targetName}?api-version=2025-01-02
     Content-Type: application/json

     { "properties": { "targetType": "SqlDb", "connectionServerName": "sql-prod.database.windows.net", ... } }
     ...
     PUT .../watchers/{name}/alertRuleResources/{ruleName}?api-version=2025-01-02
     Content-Type: application/json
     ...
     POST .../watchers/{name}/start?api-version=2025-01-02

     HTTP/1.1 202 Accepted
     Azure-AsyncOperation: https://management.azure.com/...operationStatuses/{id}
     ```

     Show actual resource paths, HTTP methods, key body fields,
     response status codes, and LRO polling headers where applicable.
     Include cross-service resource IDs (e.g., Azure Monitor, Key Vault,
     Storage) when the API references them.

   - **What the customer gains**: the concrete business outcome (e.g.,
     "the DBA now receives Teams notifications when any monitored
     database exceeds 80% CPU, with a direct link to the ADX dashboard")
   - **Cleanup**: which delete/detach operations undo the scenario
...
author can copy it into a new README.md file.
